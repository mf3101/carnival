...
' ^^^^^^^^
'   v0.11
'     (c) 2008 Simone Cingano
'		licence CC 2.5 Attribuzione Italia (http://creativecommons.org/licenses/by/2.5/it/)
'	  
'   HTTP://WWW.IMENTE.ORG/SHORT/ASPDBBOX
'-------------------------------------------------------------------------------------------
...
Class Class_ASPdBManager
	...
	public function Connect(db,user,password,options)
		...
		return = true
		openstring = createConnectionString(database,db,user,password,options)
		if openstring = null then return = false
	...
	private function createConnectionString(database_,db,user,password,options)
		dim openstring : openstring = null
		select case database_
			case "mdb"
		...
		if options <> "" and openstring <> null then openstring = openstring & options
		createConnectionString = openstring
	end function
	...
	public function CopyTables(database_,db,user,password,options)
		CopyTables = true
		dim rst, ConnFrom
		Set ConnFrom = Server.CreateObject("ADODB.Connection")
		ConnFrom.Open createConnectionString(database_,db,user,password,options)
		
		SQL = "SHOW TABLES"
		if database_ = "mysql" then
			set rst = ConnFrom.execute(SQL)
		...
		while not rst.eof and CopyTables
			if not ExecCopyTable(ConnFrom, database_,rst(0)) then
				CopyTables = False
			end if
			rst.movenext
		wend
		...
		ConnFrom.Close
		set ConnFrom = Nothing
		
	end function
	
	public function CopyTable(database_, db, user, password, options, table)
		dim ConnFrom
		Set ConnFrom = Server.CreateObject("ADODB.Connection")
		ConnFrom.Open createConnectionString(database_,db,user,password,options)
		
		CopyTable = ExecCopyTable(ConnFrom, database_,table)
	...
	private function ExecCopyTable(ConnFrom, database_,table)
		ExecCopyTable = False
		dim rcolumns, fieldsdef, fieldsvalue
		...
		dim ii : ii = 0
		while not rs.eof
			redim preserve rcolumns(2,ii)
			rcolumns(0,ii) = rs("field")
			rcolumns(1,ii) = MYSQLFieldType(rs("type"))
			fieldsdef = fieldsdef & rcolumns(0,ii) & ", "
			rs.movenext
			ii = ii+1
		wend
		fieldsdef = left(fieldsdef,len(fieldsdef)-2)
		...
		while not rs.eof
			...
			ii = 0
			fieldsvalue = ""
			while ii <= ubound(rcolumns,2)
				fieldsvalue = fieldsvalue & applyFilter(database,rs(rcolumns(0,ii)),rcolumns(1,ii)) & ", "
				ii = ii + 1 
			wend
			fieldsvalue = left(fieldsvalue,len(fieldsvalue)-2)
			
			SQL = "INSERT INTO " & table & " (" & fieldsdef & ") VALUES (" & fieldsvalue & ")"
	...
	private function MYSQLFieldType(Byval value)
	
		value = cstr(value)
		
		Reg.Pattern = "(?:varchar|char|longtext|text|shorttext)"
		if Reg.Test(value) then
			MYSQLFieldType = "string"
			exit function
		end if
	
		Reg.Pattern = "(?:datetime)"
		if Reg.Test(value) then
			MYSQLFieldType = "date"
			exit function
		end if
		
		MYSQLFieldType = "number"

	end function
	
	private function applyFilter(database,value,fieldtype)
	
		select case fieldtype
			case "string"
				applyFilter = "'" & fString(value) & "'"
			case "number"
				applyFilter = fNumber(value)
			case "date"
				applyFilter = fDate(value,database)
		end select
	...
	public function Execute(byval SQL)
		...
		on error resume next
		SQL = CompatibleSQL(SQL,database)
		set Execute = Conn.Execute(SQL)
		if err.number <> 0 then
			if debugging then call Report("EX",SQL,errorstop)
		end if
		on error goto 0
	
	end function
	
	public function ExecuteC(mdbSQL,mysqlSQL)
		dim SQL
	...
	public function CompatibleSQL(SQL,database)
			...
			case "SELECT"
				...
				if database = "mysql" then
					...
					if reg.test(SQL&" ") then
						SQL = reg.replace(SQL,"SELECT $2 LIMIT $1")
					end if
				end if
				
				'* compatible LIMIT
				if database = "mdb" then
					Reg.Pattern = "SELECT ((?:.(?!LIMIT|UNION(?: ALL)?[( ]+SELECT|[) ]+$))+.)LIMIT (\d+)(?:,\d+)?"
					if reg.test(SQL) then
						SQL = reg.replace(SQL,"SELECT TOP $2 $1")
					end if
				end if
	...
	public function fDate(byval argDate, database)
		dim tmpdateoutput
		select case database
	...
	private sub Report(what,byval exec,ending)
		dim title,description
		exec = server.HTMLEncode(exec)
		select case what
			case "NC" 'connection
			...
			description = "<strong>sql:</strong> " & exec
			case "EX" 'execute
			title = "Impossibile eseguire la query"
			description = "<strong>sql:</strong> " & exec
		end select
   ...
   public sub Reset()
	...
	public function Paginate(byref Conn, byval database, recordsPerPage, Page, pagsql)
		...
		if isnull(recordsPerPage) then
			'from querystring
			m_RecordsPerPage = inputLong(request.QueryString("perpage"))
			if m_RecordsPerPage = 0 then m_RecordsPerPage = RecordsPerPageDefault
		else
			m_RecordsPerPage = inputLong(recordsPerPage)
		end if
		if m_RecordsPerPage < 0 then m_RecordsPerPage = 0
		
		'check if no pagination
		if m_RecordsPerPage = 0 then
			godirect = true
			SQL = CompatibleSELECT(pagsql,database)
			...
			'*clean SQL
			SQL = pagsql
			SQL = CompatibleSELECT(SQL,database)
		
			if isnull(Page) then : m_Page = inputLong(request.QueryString("page")) : else : m_Page = inputLong(Page) : end if
			...
			if database = "mysql" then
				...
				if multiselectordistinct then
					...
					'INSERT COUNT
					Reg.Pattern = "SELECT (?:.(?!FROM))+ FROM"
					SQLfirst = Reg.Replace(SQL,"SELECT Count(*) AS APRecords FROM")
					'REMOVES GROUP BY AND LIMIT
					Reg.Pattern = "(?:GROUP BY (?:.(?!" & KeyWordsAfter("GROUP BY",true) & "))+|LIMIT \d+(?:,\d+)?)"
					SQLfirst = Reg.Replace(SQLfirst,"")
				end if
	...
	public function printNavigator(atext,add,queryadd,printperpage)
		...
		text = replace(text,"%LA",printNavigatorPage("pagina",m_Page,queryadd,printperpage,RecordsPerPageDefault))
		else
		lastrecord = m_Record+m_RecordsPerPage-1
		if (lastrecord>m_Records) then lastrecord = m_Records
		text = replace(text,"%RT",m_Records)
		text = replace(text,"%RE",lastrecord)
		text = replace(text,"%LA",printNavigatorPage("tutti",m_Page,queryadd,true,-1))
		end if
		...
		if m_Page > 1 then
			if m_Page > 4 then pagenav = pagenav & printNavigatorPage("&laquo;",1,queryadd,printperpage,m_RecordsPerPage)
			pagenav = pagenav & printNavigatorPage("&lsaquo;",m_Page-1,queryadd,printperpage,m_RecordsPerPage)
			if m_Page - 3 > 0 then pagenav = pagenav & printNavigatorPage(m_Page-3,m_Page-3,queryadd,printperpage,m_RecordsPerPage)
			if m_Page - 2 > 0 then pagenav = pagenav & printNavigatorPage(m_Page-2,m_Page-2,queryadd,printperpage,m_RecordsPerPage)
			if m_Page - 1 > 0 then pagenav = pagenav & printNavigatorPage(m_Page-1,m_Page-1,queryadd,printperpage,m_RecordsPerPage)
		end if
		pagenav = pagenav & "<strong>" & m_Page & "</strong> "
		if m_Page < m_Pages then
			if m_Page + 1 < m_Pages + 1 then pagenav = pagenav & printNavigatorPage(m_Page+1,m_Page+1,queryadd,printperpage,m_RecordsPerPage)
			if m_Page + 2 < m_Pages + 1 then pagenav = pagenav & printNavigatorPage(m_Page+2,m_Page+2,queryadd,printperpage,m_RecordsPerPage)
			if m_Page + 3 < m_Pages + 1 then pagenav = pagenav & printNavigatorPage(m_Page+3,m_Page+3,queryadd,printperpage,m_RecordsPerPage)
			pagenav = pagenav & printNavigatorPage("&rsaquo;",m_Page+1,queryadd,printperpage,m_RecordsPerPage)
			if m_Page <= m_Pages - 4 then pagenav = pagenav & printNavigatorPage("&raquo;",m_Pages,queryadd,printperpage,m_RecordsPerPage)
		end if
	...
	private function printNavigatorPage(text, page,queryadd, printperpage, perpage)
		dim return
		return = "<a href=""?page=" & page
		if printperpage then return = return & "&amp;perpage=" & perpage
	...
	private function inputLong(argValue)
		on error resume next
		if argValue = "" then : inputLong = clng(0) : else : inputLong = clng(trim(argValue)) : end if
		if err.number = 6 then inputLong = 0
		if err.number = 13 then inputLong = 0
		on error goto 0
	end function
	...
	public function CompatibleSELECT(SQL,database)
		...
		if database = "mysql" then
			Reg.Pattern = "SELECT TOP (\d+)((?:.(?:(?![ )]+UNION(?: ALL)?[( ]+SELECT|[) ]+$)))+.)"
			if reg.test(SQL&" ") then
				CompatibleSELECT = reg.replace(SQL,"SELECT $2 LIMIT $1")
				exit function
			end if
		end if
		...
		if database = "mdb" then
			...
			if reg.test(SQL) then
				CompatibleSELECT = reg.replace(SQL,"SELECT TOP $2 $1 $3")
				exit function
			end if
		end if
		CompatibleSELECT = SQL
